...
| `libra-builds` | `/build-output` | 构建产物、模块、`artifacts/`、模板缓存(`templates/`) |
...
| 构建报 template asset not found | Releases 无对应资产 / tag 与仓库不匹配 | 打 tag 触发模板 workflow 后 Console 刷新;或改 `LIBRA_TEMPLATE_REPO\|TAG\|BASE_URL\|TOKEN`;或在本机源码模式下构建 |
