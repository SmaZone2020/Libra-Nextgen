...
插件以 **zip 包**交付，在 Console **插件管理页** 导入/启用。完整示例与详细字段见 [`src/plugins/com.example.plugin-sdk/README.md`](../../src/plugins/com.example.plugin-sdk/README.md)（保姆级教程，前端页内也有活文档）。
...
`module/xxx.js` 入口 `function main(args)`，`args` 是服务端组装的输入（含 `op`），返回值会被 JSON 序列化作为结果：
...
沙箱为裸 QuickJS 运行时（无 `fetch`/`require`/`console`/`eval`，日志用 `log()`）；平台分支用 `__platform()`（返回 `"windows" | "linux" | "macos" | "unknown"`）**运行时**判断，平台专属函数只在对应平台注册。平台 API 速查：
...
### exec：独立子进程执行（fork-and-run，所有平台通用）

`exec.run(program, args, {env, cwd, timeoutSeconds})` 在**独立子进程**中执行程序并等待结果
（Linux = fork+exec，Windows = CreateProcessW）；`exec.spawn(program, args, {env, cwd})`
脱胎启动后台进程并立即返回 PID。子进程与 Agent 隔离：崩溃、卡死（超时自动 kill）
都不影响 Agent 本体。返回 JSON 字符串：
...
找不到程序返回 `{"success":false,"error":"..."}` 而非抛异常；`exec` 也可在
`forkexec` 云模块（MCP `execute_process`/`spawn_process`）中按相同语义使用。
