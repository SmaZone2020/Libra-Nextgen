...
pub fn validate_and_package(
    ...
    let e_lfanew = u32::from_le_bytes(
        dll_bytes[0x3C..0x40].try_into().map_err(|_| "Failed to read e_lfanew")?
    ) as usize;
    ...
    let nt_sig = u32::from_le_bytes(
        dll_bytes[e_lfanew..e_lfanew + 4].try_into().unwrap()
    );
    ...
    let machine = u16::from_le_bytes(
        dll_bytes[e_lfanew + 4..e_lfanew + 6].try_into().unwrap()
    );
    if machine != 0x8664 {
...
fn has_export(dll_bytes: &[u8], nt_offset: usize, target: &str) -> Result<bool, String> {
    ...
    let num_dirs = u32::from_le_bytes(
        dll_bytes[opt_offset + 0x6C..opt_offset + 0x70].try_into().unwrap()
    ) as usize;
    ...
    let export_rva = u32::from_le_bytes(
        dll_bytes[opt_offset + 0x70..opt_offset + 0x74].try_into().unwrap()
    ) as usize;
    let export_size = u32::from_le_bytes(
        dll_bytes[opt_offset + 0x74..opt_offset + 0x78].try_into().unwrap()
    ) as usize;
    ...
    let num_names = u32::from_le_bytes(
        dll_bytes[export_file_offset + 0x18..export_file_offset + 0x1C].try_into().unwrap()
    ) as usize;
    let names_rva = u32::from_le_bytes(
        dll_bytes[export_file_offset + 0x20..export_file_offset + 0x24].try_into().unwrap()
    ) as usize;
    let names_offset = rva_to_offset(dll_bytes, nt_offset, names_rva)?;

    for i in 0..num_names {
        let name_rva = u32::from_le_bytes(
            dll_bytes[names_offset + i * 4..names_offset + i * 4 + 4].try_into().unwrap()
        ) as usize;
        let name_offset = rva_to_offset(dll_bytes, nt_offset, name_rva)?;

        // Read null-terminated string
        let name_end = dll_bytes[name_offset..].iter().position(|&b| b == 0)
            .unwrap_or(dll_bytes.len() - name_offset);
        let name = std::str::from_utf8(&dll_bytes[name_offset..name_offset + name_end])
            .unwrap_or("");
...
fn rva_to_offset(dll_bytes: &[u8], nt_offset: usize, rva: usize) -> Result<usize, String> {
    let num_sections = u16::from_le_bytes(
        dll_bytes[nt_offset + 6..nt_offset + 8].try_into().unwrap()
    ) as usize;
    let opt_header_size = u16::from_le_bytes(
        dll_bytes[nt_offset + 0x14..nt_offset + 0x16].try_into().unwrap()
    ) as usize;
    ...
    for i in 0..num_sections {
        ...
        let raw_size = u32::from_le_bytes(dll_bytes[sec + 16..sec + 20].try_into().unwrap()) as usize;
        let raw_ptr = u32::from_le_bytes(dll_bytes[sec + 20..sec + 24].try_into().unwrap()) as usize;
        let virt_size = u32::from_le_bytes(dll_bytes[sec + 8..sec + 12].try_into().unwrap()) as usize;
...
mod tests {
    ...
    fn test_hash_known_values() {
        ...
        assert_ne!(hash_export_name("LoadLibraryA"), hash_export_name("GetProcAddress"));
    }
}
