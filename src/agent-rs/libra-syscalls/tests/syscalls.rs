...
use libra_syscalls::{
    init, nt_delay_execution, probe_stub, spoof_call, types::STATUS_SUCCESS, SyscallTable,
};
...
fn table_enumerates_nt_exports() {
    ...
    assert!(
        table.entries.len() >= 40,
        "expected many Nt* exports, got {}",
        table.entries.len()
    );
...
fn probe_extracts_ssn_and_trampoline() {
    ...
    assert_eq!(
        bytes,
        &[0x0F, 0x05],
        "trampoline must point at syscall opcode"
    );
}
...
fn init_then_invoke_real_syscall() {
    ...
    assert_eq!(
        ...
        "NtDelayExecution should succeed, got {status:#x}"
    );
}
...
fn spoof_call_real_api() {
    ...
    unsafe {
        ...
        assert_eq!(
            pid as u32,
            std::process::id(),
            "spoofed call must return real pid"
        );
    }
}
